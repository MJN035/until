...
from .context.bundle import assemble_context
# 공개 API 재수출(하위 호환: `from .pipeline import Result`가 여전히 동작해야 한다).
# 실제 정의는 `.types`에 있다 — MCP 경로가 `pipeline.py`(그리고 `llm.base`)를
# 로딩하지 않고 `Result`만 쓸 수 있게 하는 게 목적이다.
