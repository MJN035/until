...
| **MCP 서버**(`python -m until.mcp_server`, stdio JSON-RPC. 읽기 전용 9도구 — inbox·assignment·materials·route·readiness·series·control_tower·semester·brief. 생성 도구 없음·LLM 0·의존성 0·토큰 미저장) | `until/mcp_server.py` | test_mcp_server |
| **`until-mcp setup`**(Claude Code·Codex CLI 설정에 MCP 서버 등록만 — 토큰은 묻지도 쓰지도 않음, 기존 설정 덮어쓰지 않음, 이미 등록돼 있으면 그대로 둠. `packaging/npm/lib/setup.js`와 동작 동일) | `until/setup.py` | test_setup |
...
```bash
PYTHONIOENCODING=utf-8 python run_tests.py      # 90스위트 병렬(~15s). -j 1 = 순차
